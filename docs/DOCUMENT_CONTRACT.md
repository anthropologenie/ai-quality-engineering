...
**Status:** Planning (Awaiting Review — Sprint P2.5)
...
| No component named "Loader" or equivalent appears in `docs/architecture.md` §5's Component Architecture table | The responsibility of turning a raw source file into `Document.text` is real (the Chunker's input must come from somewhere) but is not currently owned by any named, frozen architectural component. |
...
- **Unclear ownership:** no component in `docs/architecture.md` §5 is named as owning the text-extraction step (`.docx` → plain text) that `Document.text` presupposes. `KnowledgeSource` is named as the producer of `Document` values, but the mechanism is unaddressed by any frozen document.
...
| **Loader** *(unnamed in the current architecture)* | The mechanism that would populate `Document.text` from a raw source file (e.g., `.docx` parsing) has no owning component named in `docs/architecture.md` §5. This is a genuine boundary gap, not resolved by this contract — recorded in Phase 11. | Absence confirmed by repository scan (`docs/CHUNK_BUILDER_IMPLEMENTATION_PLAN.md` §2; `requirements.txt`) |
...
- **Loader ownership** — naming and freezing which component owns text-extraction is an architecture-level decision (would touch `docs/architecture.md` §5, a currently locked document) rather than a Document-contract-level one. Recorded as Outstanding Question 2 (Phase 11), not resolved here.
...
2. **Which component owns text-extraction (e.g., `.docx` parsing) — is a new "Loader" component warranted, or does this responsibility belong inside `KnowledgeSource` itself?**
   - Why unresolved: `docs/architecture.md` §5's Component Architecture table names no such component; only `KnowledgeSource` is named as producing `Document` values, without detailing the mechanism.
   - Construction evidence required: this is an architecture-level question, not resolvable within a Contract Freeze sprint's scope (Phase 6/7 were correctly not executed here). Answering it may require a deliberate, narrowly-scoped architecture decision, following `docs/architecture.md` §10's own governance rule that locked decisions are revisited only via "a deliberate redesign discussion, not an incidental change made while implementing a later milestone."
   - Disposition: recorded as a genuine gap, belongs to a future sprint — likely immediately preceding Document Construction, since Construction cannot proceed without it.
...
3. **Does JobOps structured data (SQLite) ever get modeled as a `Document`, or does SQL-filter retrieval permanently bypass `Document`/`Chunk` entirely?**
   ...
   - Disposition: intentionally not resolved by this contract (Document's proposed schema, Section 8.2, is scoped to text corpus items only — resume and job descriptions — consistent with `docs/glossary.md` §8's own definition). Flagged for a future milestone or documentation clarification, not a blocking issue for Document Construction against text-based sources.

4. **Should `Document` be classified under the Persistent Canonical Artifact / Runtime Artifact distinction `docs/CHUNK_CONTRACT.md` §5 introduced (as a recommendation, not yet a glossary term)?**
   - Why unresolved: that classification was itself a recommendation, not a frozen glossary entry (`docs/CHUNK_CONTRACT.md` §3). Whether `Document` fits either category cleanly depends on the still-open Serialization question (Phase 10).
...
None of the above block the field-level contract in Phase 8 from being reviewed and frozen. Each is a forward dependency for later work, not a defect in this document's own scope.
...
**Result: PASS**, pending human review. No blocking gap was found for this planning sprint itself. Four outstanding questions are recorded (Phase 11), none of which block review of the proposed two-field contract (Section 8.7) — each is either scoped to a later sprint (Document Construction, Serialization, Validation) or to a separate architecture-level decision (Loader ownership, JobOps classification) outside this Contract Freeze's authority.
...
Awaiting review and approval of the proposed contract (Section 8.7) before a Document Construction sprint begins.
