...
| **M2-08** | DeepEval activation — Layer 3 | Evaluation Tooling | **Blocks 2** | Does not block Milestone 1A | **RO-18** | `docs/roadmap.md` §5; `docs/altm.md` §9, §12 | ▶ **IMPLEMENTATION PATH RESCOPED BY RO-18 (§4.10) — read this row's *Capability* name as the original mechanism label, not as the current one.** The capability, its identifier **M2-08**, its class, its blocking status, its milestone (**2**) and its ownership are **unchanged**, as is every other column of this row; **only the implementation mechanism is rescoped**, and **no new `M2-xx` was created and nothing was renamed**. **M2-08 is now authorized to implement a native, repository-owned computation of Faithfulness, Groundedness, Hallucination Rate and Answer Relevancy** — the set fixed at exactly four by **RO-18 Decision 1**, with none added and **none dropped because an external framework does not provide it**, and with the semantics derived from the repository's own authoritative metric definitions rather than from framework terminology (**Decision 8**). **DeepEval-the-library is NOT adopted for M2-08 under the current repository constitution** (**Decision 2**): **no release in its full published history implements Groundedness**, which `docs/AI_Quality_Metrics_Reference.md` holds at **Primary**, and the releases carrying the remaining metrics declare `langchain` or `openai` among their core, non-extra dependencies — **NA-07** and **RO-16 D-5** respectively. **NA-07 is preserved unchanged and no fifth A-5 exception was created**; nothing is installed, no `import deepeval` exists and `requirements.txt` is byte-identical. **RO-18 Decision 4 independently and narrowly authorizes the existing `sample_rag/deepseek.py` boundary as the M2-08 judge seam**, on conditional technical reuse without architectural widening — **not** by extending **RO-17 D-4**, whose scope stays M2-07-only — and **RO-14 D-2 remains binding: no provider call may enter the deterministic pytest suite.** **RO-16 D-3 is not rewritten**: its *"for M2-08 automatically"* DeepSeek exclusion is superseded on that single point by **RO-18 Decision 5**, and every other RO-16 D-3 restriction stands. **Promptfoo is not part of M2-08, and M3-01 remains OPEN** (**Decision 3**). **RO-18 discharges nothing and grants no new dependency, provider, embedding, vector-store, model-router or orchestration authority** — **M2-08**, **M2-10**, **M3-01** and **M3-06** all remain **OPEN**, and **M2-18** stays ✅ DISCHARGED and untouched. ▼ *The original rationale below is retained as written.* Measures the Infer stage; meaningless against a verbatim-quotation generator — §4.2 |
...
> **▶ Synchronized after commit `42bcadb` — the paragraph above is retained as written.** It was accurate when RO-18 issued it and **CP-3** governs: **at commit `6543bae`, the checkpoint the paragraph itself names, all four artifacts were untouched**, and this note does not represent any later change as having existed at the time of the ruling. **`scripts/evaluate_context.py` was subsequently modified, at commit `42bcadb`** — *"fix(evaluation): add safe CLI selection guard to M2-07 evaluator"* — a **later implementation change that RO-18 neither authorized nor anticipated**, made after it. **What changed is the evaluator's invocation surface alone:** the script now requires an explicit selection (`--case-id ID`, or `--all`) and defers its two expensive imports until that selection has been validated, so an invocation carrying no selection, an unknown case id, an unknown argument or `--help` is refused before any model, index or provider object exists. **What did not change, and is verifiable across `6543bae`…`42bcadb`:** the **Context Precision / Context Recall semantics are unchanged**; `evaluation/context_metrics.py` and `tests/test_context_metrics.py` are **byte-identical** and remain untouched exactly as the paragraph states; the retrieval the evaluator measures is unchanged, the executable bodies of `load_reference_entries`, `retrieve_cases`, `partition` and `report_scores` being identical apart from the relocated imports; and `docs/M2.07_Native_Context_Metrics_Report.md` is **unedited**, under **CP-3**. **The remainder of Decision 11 stands unamended** — M2-07 is not reopened, not redesigned and not rerun, **RO-17 is unamended in every part**, no retrieval optimization is authorized, and **M2-07 remains OPEN**. **This note creates, amends and renumbers no ruling**, edits no capability row, and discharges nothing; it is the adjacent record, in the manner §4 already uses for a statement overtaken by a later commit. Evidence: `git show 42bcadb --stat` — two files, `scripts/evaluate_context.py` and `tests/test_evaluate_context_cli.py`; `git diff 6543bae..HEAD -- evaluation/context_metrics.py tests/test_context_metrics.py docs/M2.07_Native_Context_Metrics_Report.md`, empty; `docs/ENGINEERING_TRACEABILITY_REGISTER.md` §5.
...
> **▶ Applied by the Repository Owner at commit `ce779a7` — the Finding above is retained as written and is not edited.** Its language was **conditional, and the condition has since been met**, so the Finding was accurate when issued rather than false: commit `ce779a7` — *"docs(governance): synchronize M2-08 row with RO-18"*, the commit immediately following RO-18's own recording at `3a068fb` — **applied the proposed synchronization to §4's canonical M2-08 row**, exactly as proposed and in nothing more. Verified column by column across `ce779a7^`…`ce779a7`: the *Ruling* column moved from **`—`** to **RO-18**; the *Rationale* column gained the proposed **▶ rescoping marker** as a prefix, with the original rationale — *"Measures the Infer stage; meaningless against a verbatim-quotation generator — §4.2"* — **retained verbatim at its tail**; and the **identifier, Capability name, class, blocking status, P3.7.2 class and originating authority are byte-identical**, as are all other rows. **No capability was created, renamed, merged or discharged**, and **M2-08 remains OPEN**. The row therefore now carries the applied language, and a reader no longer needs this Finding to resolve it — the Finding remains as the record of why the synchronization was proposed rather than performed by RO-18 itself. **This note creates, amends and renumbers no ruling** — RO-18 stands as issued in every part, **RO-10 Decision 2 is neither relaxed nor reinterpreted**, and nothing here supplies or implies a row-editing authorization. Evidence: `git show ce779a7 --stat` — one file, one insertion, one deletion; the M2-08 row at HEAD is byte-identical to the row `ce779a7` wrote.
