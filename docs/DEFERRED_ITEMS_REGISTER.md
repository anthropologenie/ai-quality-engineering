...
| **1B-13** | **O-5** — corpus-scale vacuity of DQ-2 / DQ-4 | Validation | Non-blocking — trigger-bound | *(not in §5)* | A7 | `docs/DATA_QUALITY_VALIDATION_PLAN.md` §16 O-5 | Recorded trigger is *"corpus expansion"*, performed by 1B-05 / 1B-06 — §4.1 R-1B-13/14/15/16 |
| **1B-14** | **I-6** — `test_b6` hardcodes `Karthik_SR_Resume_v2_2.docx` | Validation | Non-blocking — trigger-bound | Does not block Milestone 1A | — | `docs/ENGINEERING_TRACEABILITY_REGISTER.md` §3.5 | Disposition is *"re-verify at corpus expansion"* — the same trigger — §4.1 R-1B-13/14/15/16 |
...
**5 items.** Two were discharged by Sprint P3.7.4 and are retained here with their discharge recorded, per §1.3 (*a capability is never deleted*).

| id | Item | Class | Blocking status | P3.7.2 class | Auth. | Originating repository authority | Status |
|---|---|---|---|---|---|---|---|
| **RO-01** | Canonical Document Marking — version preference between resume v2.2 and v2.3 | Governance + Corpus | **Blocks 1A Closure** | Blocks Closure | — | `docs/GENERATION_CONTRACT.md` §21, §23 Q-5; `docs/P3.7.2_…` §6.6 | **Open, and now unsequenced** — see §9. Repository verified ready for it; blocked by nothing — §4.4 R-RO-01 |
| **RO-02** | `ALTM-KNOWLEDGE-1` determinability — *"wrong document version"* | Diagnostic | Non-blocking — trigger-bound | Does not block Milestone 1A | — | `docs/P3.3.5_…` §4; `docs/P3.3.4_…` §5, §7.1 | **Open.** Follows RO-01 automatically; requires no engineering — §4.4 R-RO-02 |
...
| **RO-05** | `datasets/synthetic/` purpose decision | Governance | **Blocks 1B start** | *(not in §5)* | — | `docs/architecture.md` §11; `datasets/README.md` | **Open.** Must be decided *before* Milestone 1B writes corpus data, or the locked Golden Dataset design is at risk — §4.4 R-RO-05 |
