...
**16 capabilities.** Eight reassigned from Milestone 1A by `docs/P3.7.3_…` Decision 5; eight affirmed here without ever having been Milestone 1A obligations. **Two — 1B-08 and 1B-09 — were discharged by Sprint 1B.1 and are retained here with their discharge recorded**, per §1.3 (*a capability is never deleted*). The count is unchanged for that reason: it counts capabilities allocated, not capabilities outstanding.
...
| **1B-08** | **DQ-5** — chunk validity as a corpus property | **Sprint 1B.1 (Corpus Integrity)**, commit ⟨**PENDING — Repository Owner to record at commit time**⟩ | `tests/test_data_quality.py` — plan §11.2 phase **W6**: the committed-collection contract gate, the §17 six-field completeness check, and the §17 positional-derivation check for `chunks[].id`; `tests/conftest.py` `real_chunk_collection` |
| **1B-09** | **DQ-6** — chunk referential integrity, incl. Chunk invariant 3's full form | **Sprint 1B.1 (Corpus Integrity)**, commit ⟨**PENDING — Repository Owner to record at commit time**⟩ | `tests/test_data_quality.py` — plan §11.2 phase **W6**: `chunks[].document_id` → Manifest entry and → loaded `Document`; Manifest-side coverage; and `docs/CHUNK_CONTRACT.md` §17 invariant 3 in full form, `text == document_text[character_start:character_end]` |

**Commit reference.**

- **RO-01, RO-02, RO-05** are carried by commit **`e5d7ce0`** — *docs(repository): synchronize corpus and establish Milestone 1B engineering baseline*.
- **1B-08 and 1B-09** are discharged by a working-tree change under Repository Owner review; **the commit that carries it does not yet exist.** This follows the precedent `docs/P3.7.6_…` §3 set for the same situation — *"That commit does not yet exist — it is the commit that will add this document."* **The placeholder above is to be replaced with the commit hash at commit time.** The evidence column cites the specifications, which are verifiable independently of the hash.
