...
## 25. Two v2.0.0 Readings, Resolved — Contract Erratum E-1

*Added under Repository Owner ruling **RO-14 — Generator / ModelGenerator Architectural Identity** (`docs/DEFERRED_ITEMS_REGISTER.md` §4.6), Decisions 5 and 6. **Sections 1 through 24, the v1.0.0 closing statement and the header block above are byte-for-byte unchanged.** This section resolves two readings that §24 admits of its own text. It **adds no field, removes no field, changes no type, alters no guarantee, withdraws no deferral, and authorizes no implementation.** Sections 1–23 remain **v1.0.0** and §24 remains **v2.0.0**; **no new contract version is created** — there is no v2.1.0 and no v3.0.0, and the version metadata above is unchanged. **No runtime file is changed by this section**, and it makes no quality claim of any kind.*

**Why an erratum rather than an edit.** `docs/DOCUMENT_CONTRACT.md` §8.9 (**Erratum E-1**) established the repository's mechanism for a contract that *"admits two readings of the same requirement"*: an adjacent section that *"resolves that, without editing either statement."* §8.10 (**Erratum E-2**, under ruling **R-02**) established the companion form for a citation whose cited authority has since changed — the citation is *"superseded as a statement of the current schema"* while *"each remains accurate as the observation it was when recorded, and none is edited."* §24 itself was added through the same adjacent, non-destructive mechanism. **Both readings below are resolved through it, and through no other**; rewriting §8.1 or §16 in place would make v1.0.0 claim something the Milestone 1A component never did, destroying the record of what was actually built and validated.

### 25.1 `answer_text` under v2.0.0 — the resolution of M2.06-F-2

**The two readings.** §24.3 states that *"§7's data model, §8's field definitions, §9's outcome domain, §10's traceability requirements, §11's ordering semantics and §13's serialization form"* are unchanged. §8.1 defines `answer_text` on the Answer path as *"assembled from the `statements` below and contains no content not present in them."* The same §24.3 states, specifically and by name, that under v2.0.0 `answer_text` *"**MAY be synthesized by the model**"* and that *"the quotation-only derivation requirement no longer applies to it."* Surfaced as finding **F-2** of `docs/M2.06_Generation_Report.md` §6.2 and reported there for confirmation. **RO-14 Decision 5 resolves it.**

**1 — The specific G-7 transition governs.** Under **v2.0.0**, `answer_text` **MAY be synthesized by the model**. **§16 is where guarantees are normative** — its own preamble states that *"a `GenerationResult` that violates any guarantee below does not conform to this contract"* — and §24.3 names **G-7** as one of the four guarantees that transition. §8.1's clause is the **descriptive restatement** of G-7's quotation-only derivation requirement, and it does not survive the transition of the guarantee it restates.

**2 — The field itself is unchanged, and that is what §24.3 preserves.** `answer_text` keeps its **name**, its **type** (`str`), its **required** status, its position in §7's declaration order and therefore in §13.2's serialized key order, and **G-3**'s non-emptiness on every path including Abstain. §8.1's Abstain clause — a fixed abstention text asserting nothing about the corpus (§9.3) — is **unchanged and remains binding** under v2.0.0. *"§8's field definitions unchanged"* is accurate as to the field; it does not extend to a guarantee §24.3 separately and expressly transitions.

**3 — §8.1 is not edited, and remains accurate for v1.0.0.** The quotation-only behaviour it describes is a true statement about the Milestone 1A deterministic quotation `Generator`, a component that **still exists, is still executed by `scripts/cli.py`, and still exhibits exactly that behaviour** (RO-14 Decision 1). §8.1 is superseded **only as a statement of v2.0.0 `answer_text` behaviour**, and in no other respect.

**4 — The surviving half of G-7 is unchanged.** `GeneratedStatement.text` remains a **verbatim quotation of its own span** under both eras. **`SupportingEvidence` SHALL remain grounded in the assembled `Prompt` context and its provenance, and in nothing else**, and the v2 Generator **SHALL NOT** obtain evidence from an independent retrieval or corpus path. **G-4**, **G-6**, **G-8**, **G-10**, **G-11** and **G-12** are unaffected.

**5 — No quality guarantee is created by this resolution.** §24.3's three-way distinction stands in full: **structural evidence provenance** and **model answer synthesis** are established by the contract; **empirical Faithfulness / Groundedness** is *"later evaluation work, neither established nor claimed here."* §21's exclusion of the Layer 3/4 metric set stands, and no Faithfulness, Groundedness, Hallucination Rate, Answer Relevancy, Context Precision or Context Recall claim is authorized. **That a synthesized `answer_text` is permitted is not a statement that it is faithful.**

**6 — The implementation is confirmed, not corrected.** `sample_rag/model_generator.py` already applies the reading recorded here. **No runtime change, no test change and no implementation authorization follows from this subsection.**

### 25.2 G-5 under v2.0.0 — evidence identity through `Prompt.provenance`

**The observation.** §24.3 lists **G-5** among the guarantees *"unchanged in every part"*. §16's G-5 reads: *"Every `SupportingEvidence` SHALL carry a `chunk_id` present in the consumed `RetrievalResult.chunks`, and a `document_id` equal to that chunk's `document_id`."* But §24.3's **G-13 / §18** transition makes a **`Prompt`** the v2 Generator's permitted input, and §24.2 states that the v2 Generator *"SHALL NOT consume a `RetrievalResult`."* The literal wording therefore names an artifact the v2 component is structurally barred from consuming. **RO-14 Decision 6 resolves it.**

**1 — G-5's intent is binding under v2.0.0.** Every `SupportingEvidence` **SHALL** carry a `chunk_id` that resolves to a real chunk of the committed Chunk Corpus, and a `document_id` equal to that chunk's own `document_id`. **That requirement is unchanged, and it is what G-5 has always been for**: evidence identity that resolves, rather than an identifier the Generator asserted.

**2 — The `RetrievalResult.chunks` phrase is a v1.0.0 input citation.** It names where a conforming `chunk_id` came from **under the v1.0.0 input boundary**, where the Generator's two permitted inputs were the query and a `RetrievalResult`. It is accurate as the statement it was when recorded, and it is **superseded as a statement of the v2.0.0 Generator boundary** — the disposition §8.10 of `docs/DOCUMENT_CONTRACT.md` applies to a citation whose cited contract has since changed. **§16 is not edited.**

**3 — Under v2.0.0 the requirement is satisfied through `Prompt.provenance`.** §24.4 authorizes an ordered per-chunk structure carrying **exactly** `chunk_id`, `document_id`, `character_start` and `character_end`, in assembled chunk order. **U-2** established that extension precisely because `SupportingEvidence` (§8.3) could not otherwise be constructed once **G-13** barred the Generator from reaching back into the corpus. The `document_id` G-5 requires is therefore carried **beside** its `chunk_id`, in one ordered record, which makes the equality G-5 states hold **by construction** rather than by lookup.

**4 — A `RetrievalResult` remains the upstream source, not a v2 input.** §24.2's pipeline is `retrieval -> ordered chunk ids -> ContextBuilder -> Prompt -> Generator -> provider -> GenerationResult`. Retrieval still originates every `chunk_id` the artifact carries; what changed is that the v2 Generator receives them **through the assembled `Prompt`**. **This subsection does not make a `RetrievalResult` a v2 Generator input**, does not widen §18's permitted inputs, and does not weaken §24.2's *"SHALL NOT consume a `RetrievalResult`."*

**5 — G-6 is unchanged and remains the corpus-membership guarantee.** Every `chunk_id` carried by a `GenerationResult` SHALL exist in the committed Chunk Corpus, and it continues to hold **by construction** for the reason G-6 already records: chunk ids are carried through, and *"the Generator never constructs a chunk id."* **No identifier in a v2 artifact has passed through the provider.**

**6 — Nothing is fabricated and nothing is authorized.** No evidence, span, offset or identifier is invented by this subsection; no runtime code changes; no specification changes; and the existing implementation already satisfies the intent recorded here. **This subsection records why it does; it does not authorize any modification to make it so.**

### 25.3 What this section does not do

It **adds no field** to `Prompt`, `GenerationResult`, `GeneratedStatement` or `SupportingEvidence`, and **retypes and removes none**. It **alters no guarantee of §16** and **transitions none** — the four §24.3 transitions are unchanged, and **G-1…G-6, G-8, G-10, G-11 and G-12 remain unchanged in every part**. It **creates no contract version**, **no second authoritative location**, **no `contracts/` directory** and **no new storage or versioning mechanism**. It **does not resolve U-3**, does not widen `evaluation/altm_rules.py` `REACHABLE_STAGES`, and authorizes **no orchestration layer, runtime adapter, pipeline coordinator or `ContextEngine`**. It **selects no provider, library, endpoint, model identifier or request format**, and **adds no dependency**. It **activates no evaluation tooling** — **M2-07**, **M2-08** and **M3-01** are untouched. It **discharges nothing** — **M2-06** and **M2-14** both remain **OPEN** — and it **implements nothing**: `sample_rag/generator.py`, `sample_rag/model_generator.py`, `sample_rag/context_builder.py`, `scripts/cli.py`, `scripts/run_generation.py` and every `tests/` module are unchanged.

---

*Section 25 is a clarification of Sections 8 and 16 as they read under v2.0.0, authorized by Repository Owner ruling **RO-14**. Sections 1–23 remain Generation Contract v1.0.0, Section 24 remains v2.0.0, and both are unedited.*
