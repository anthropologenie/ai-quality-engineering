...
| **Chunk Contract** | The frozen structural contract for the `Chunk` entity — required fields, identity, parent-document relationship, ordering, and character-offset guarantees every `Chunk` must satisfy. Canonical specification: `docs/CHUNK_CONTRACT.md`. | `CHUNK_CONTRACT.md` |
...
| **Document Contract** | The frozen structural contract for the `Document` entity — a full source item exposed by `KnowledgeSource.load()` before chunking (§8) — fixing its two required fields (`id`, `text`), the identity rule binding `id` to the Knowledge Manifest's `documents[].id`, and the determinism guarantee every `Document` must satisfy. Canonical specification: `docs/DOCUMENT_CONTRACT.md`. | `DOCUMENT_CONTRACT.md` |
| **Embedding Provider** | The interface (`EmbeddingProvider`) responsible for converting text into vector representations. Defined in Milestone 1B; implemented for real (BGE-small-en-v1.5) in Milestone 2. | `architecture.md` |
...
| **Persistent Canonical Artifact / Runtime Artifact** | Two artifact lifecycles distinguished by lifetime: a Persistent Canonical Artifact (e.g. Knowledge Manifest, Chunk) is corpus-derived, deterministic, and version-controlled; a Runtime Artifact (e.g. `RetrievalResult`) is query-derived and exists only for the duration of one request. | `CHUNK_CONTRACT.md` |
